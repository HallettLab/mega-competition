...
parameters{
  ...
  // all the alphas
   real<lower=0> alpha_acam_base;
   real<lower=0> alpha_amme_base;
   real<lower=0> alpha_anar_base;
   real<lower=0> alpha_brho_base;
   real<lower=0> alpha_brni_base;
   real<lower=0> alpha_ceso_base;
   real<lower=0> alpha_gitr_base;
   real<lower=0> alpha_leni_base;
   real<lower=0> alpha_lomu_base;
   real<lower=0> alpha_mael_base;
   real<lower=0> alpha_mica_base;
   real<lower=0> alpha_pler_base;
   real<lower=0> alpha_plno_base;
   real<lower=0> alpha_taca_base;
   real<lower=0> alpha_thir_base;
   real<lower=0> alpha_twil_base;
   real<lower=0> alpha_weeds_base;
...
model{
  ...
  for(i in 1:N){
    ...
    F_hat[i] = N_i[i]*(lambda_base + lambda_dev*trt[i]) / (1 +
    acam[i]*(alpha_acam_base + alpha_acam_dev*trt[i]) + 
