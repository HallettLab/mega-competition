...
# Prep DF ####
## get rid of interaction coeff with species that are not done processing yet
#rm <- c("alpha_brni", "alpha_leni", "alpha_clpu")

## calc mean alpha values
#mean_alphas <- posteriors_long %>%
 # filter(!alpha_name %in% rm) %>%
...
#colnames(mean_alphas) <- c("treatment", "species", "lambda", "ACAM", "AMME", "ANAR", "AVBA", "BRHO", "CESO", "GITR", "LOMU", "MAEL", "MICA", "PLER", "PLNO", "TACA", "THIR", "TWIL")
