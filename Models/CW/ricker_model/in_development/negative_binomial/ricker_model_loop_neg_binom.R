...
for(i in species){
  for(j in trt){
    ...
    model.output[[paste0("seeds_",i,"_",j)]] <- stan(file = paste0("Models/CW/ricker_model/in_development/negative_binomial/18_species_Ricker_model_",j, "_neg_binom.stan"), data = c("N", "Fecundity", "intra", "intra_g", "mean_ctrl_seeds", "sd_ctrl_seeds", "brho", "pler"), iter = 5000, chains = 4, thin = 3, control = list(adapt_delta = 0.95, max_treedepth = 20),
                                                     init = initials1) 
    
    tmp <- model.output[[paste0("seeds_",i,"_",j, "_neg_binom")]] 
    
    save(tmp, file = paste0("Models/CW/ricker_model/in_development/negative_binomial/posteriors/seeds_",i,"_",j,"_posteriors_Ricker_meanLpriors_neg_binom_", date, ".rdata"))
  }
}

#"acam", "amme", "anar", #  "brni", "ceso", "gitr", "leni", "lomu", "mael", "mica",  #"plno", "taca", "thir","twil","crco", "erbo", "figa", "gamu", "hygl", "siga", "other"
