...
model{
  ...
  alpha_brni_dev ~ normal(0, 1000); // thsi was brni_base rather than _dev, maybe it was an issue that brni_dev didn't have a prior?
  ...
  for(i in 1:N){
    ...
    F_hat[i] = N_i[i]*g_i[i]*(lambda_base+ lambda_dev*trt[i])* //g_i[i]*
