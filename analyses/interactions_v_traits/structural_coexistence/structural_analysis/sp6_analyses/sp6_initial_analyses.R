...
fdiv6 = read.csv(paste0(file_path, "calc_comm_attributes/sp6_fdiv.csv")) %>%
  select(-X)

fdiv6$comp = as.character(fdiv6$comp)
...
# Format Data ####
sp6fdiv = left_join(sp6_clean, fdiv6, by = c("comp", "ACAM", "AMME", "ANAR", "BRHO", "BRNI", "CESO", "GITR", "LENI", "LOMU", "MAEL", "MICA", "PLER", "PLNO", "TACA", "THIR", "TWIL"))

sp6fdiv$niche_diff = as.numeric(sp6fdiv$niche_diff)
