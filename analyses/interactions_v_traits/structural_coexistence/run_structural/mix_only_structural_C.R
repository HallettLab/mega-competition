...
invsp = c("ANAR", "BRHO", "BRNI", "CESO", "LOMU", "TACA", "THIR")
natsp = setdiff(sp, invsp)

## create all possible combinations of composition at a given richness level
allcomp4 <- data.frame(comboGeneral(sp, m=4, freqs = 1))
invcomp4 <- data.frame(comboGeneral(invsp, m=4, freqs = 1))
natcomp4 <- data.frame(comboGeneral(natsp, m=4, freqs = 1))
#test = anti_join(allcomp4, invcomp4)
temp = anti_join(allcomp4, natcomp4)
comp4 = anti_join(temp, invcomp4) 

## create empty dataframe
mixcommC <- data.frame(ACAM = NA, AMME = NA, ANAR = NA, BRHO = NA, BRNI = NA, CESO=NA, GITR = NA, LENI = NA, LOMU= NA, MAEL = NA, MICA = NA, PLER = NA, PLNO = NA, TACA= NA, THIR= NA, TWIL = NA, feasibility=NA, niche_diff = NA, fitness_diff = NA, draw = NA)
...
for(j in 1:nrow(comp4)){
  ...
  for (i in 1:length(draws)) {
    ...
    mixcommC <- rbind(mixcommC, temp)
    
  }
}

write.csv(mixcommC, "analyses/interactions_v_traits/structural_coexistence/run_structural/mix_C_structural_results_20240801.csv")
