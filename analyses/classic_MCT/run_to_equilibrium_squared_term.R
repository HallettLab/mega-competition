...
## Read in Data ####
## model posteriors
source("Models/CW/ricker_model/random_effects_block/negative_binomial/squared_term/posterior_processing.R")
...
for(i in 1:length(names(dry))) {
  ...
  intra_sq <- paste0("alpha_", tolower(substr(names(dry)[i], 1, 4)), "_sq")
  ...
  for(t in 1:(time-1)) {
    ...
    alpha_intra_sq <- all_intras_sq[posts]
    ...
    N[i, ,t+1] <- run.to.equilibrium.facilitation(germ = datset$germ, 
                                     surv = datset$surv,
                                     lambda = lambda, 
                                     alpha_intra = alpha_intra, 
                                     alpha_intra_sq = alpha_intra_sq,
                                     Nt = N[i, ,t], 
                                     alpha_inter = 0,
                                     germ_inter = 0,
                                     inter_abund = 0) 
...
for(i in 1:length(names(wet))) {
  ...
  intra_sq <- paste0("alpha_", tolower(substr(names(wet)[i], 1, 4)), "_sq")
...
residents_dry_long <- residents_dry %>%
  mutate(num = 1:length(ACAM)) %>%
  pivot_longer(2, names_to = "species", values_to = "equil_abund") %>%
  select(-V1)
...
normal <- ggplot(residents_dry_long[residents_dry_long$equil_abund < 600,], aes(x=num, y=equil_abund)) +
  geom_point() +
  facet_wrap(~species, scales = "free") +
  xlab("Run Number") + ylab("Eq Abund (outliers filtered out)") +
  ggtitle("Drought Conditions, Equilibrium, Nt1 = 100,  02/04/2023")

runaway <- ggplot(residents_dry_long[residents_dry_long$equil_abund > 600,], aes(x=num, y=log(equil_abund))) +
  geom_point() +
  facet_wrap(~species, scales = "free") +
  xlab("Run Number") + ylab("Log Eq Abund (outliers)") +
  ggtitle("Drought Conditions, Equilibrium, Nt1 = 100, 02/04/2023")

all_log <- ggplot(residents_dry_long, aes(x=num, y=log(equil_abund))) +
  geom_point() +
  facet_wrap(~species, scales = "free") +
  xlab("Run Number") + ylab("Log Eq Abund (all runs)") +
  ggtitle("Drought Conditions, Equilibrium, Nt1 = 100, 02/04/2023")

ggarrange(normal, all_log, runaway, nrow = 3, ncol = 1)

ggsave(paste0("analyses/classic_MCT/preliminary_equil_abundance/", date, "/ACAM_equil_abund_ricker_negbinom_sq_term_D_Nt100_", date, ".png"), height = 8, width = 6)
