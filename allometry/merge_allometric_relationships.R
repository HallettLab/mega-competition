...
## merge all together
allo.df <- do.call("rbind", list(ACAM.allo.output, ANAR.allo.output, BRHO.allo.output, CESO.allo.output, GITR.allo.output, MAEL.allo.output, MICA.allo.output, PLER.allo.output, PLNO.allo.output, TACA.allo.output, THIR.allo.output)) %>%
  mutate(poly = ifelse(is.na(poly), 0, poly)) ## change NAs to 0s, will work better in later calculations.


## clean up env
rm(list = c("ACAM.allo.output", "ANAR.allo.output", "BRHO.allo.output", "CESO.allo.output", "GITR.allo.output", "MAEL.allo.output", "MICA.allo.output", "PLER.allo.output", "PLNO.allo.output", "TACA.allo.output", "THIR.allo.output"))
