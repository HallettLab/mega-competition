...
## Visualize ####
### linear ####
final3 <- ggplot(thir_flowers_allo, aes(x=total.biomass.g)) +
  geom_histogram() +
  xlab("Aboveground Biomass (g)") +
  ylab("Count")
  
final4 <- ggplot(thir_flowers_allo, aes(x=total.biomass.g, y=all.flowers.num)) +
  geom_point() +
...
## visualize ####
final1 <- ggplot(thir_viability_allo, aes(x=viability)) +
  geom_histogram() +
  #facet_wrap(~treatment) +
  xlab("Proportion Viable Flowers") +
  ylab("Count")

final2 <- ggplot(viability_sum, aes(x=treatment, y=mean.via)) +
  geom_point(size = 3) +
...
nrow(thir_viability_allo[thir_viability_allo$treatment == "C",])
nrow(thir_viability_allo[thir_viability_allo$treatment == "D",])
...
rm(list = c("mean_viability", "viability_test", "viability_sum", "thir_viability_allo", "thir_flowers_allo", "thir_fallo_lin", "thir_fallo_lin2", "thir_combined", "thir_phyto", "final1", "final2", "final3", "final4", "plot"))
