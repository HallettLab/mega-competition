...
mica <- read.xlsx(paste0(lead, "Phytometer-Processing/", date, "_Phyto-Processing.xlsx"), sheet = 13)
...
## Check all Columns ####
### ID info ####
unique(mica$bkgrd) ## there are some here in an ERBO background.
...
unique(mica$phyto.n.indiv) ## 1-3, good
unique(mica$`complete?`) ## need to standardize capitalization
...
mica_complete_nas_check <- mica %>%
  filter(is.na(`complete?`))
    ...
    ## 5 samples need to be checked for completeness
          ## 8-6-14; 16-4-3; 16-7-3; 16-11-3; 16-13-3
## all of these samples should be removed and added back in only if they are found to be okay.
...
unique(mica$total.biomass.g) ## some NAs present
...
unique(mica$seed.num)
sort(unique(mica$seed.num)) ## varies from 3-2817
...
mica_temp <- mica %>%
  ...
  #filter(!(block == 12 & plot == 9 & sub == 8)) %>%## specifically remove only the incomplete sample
