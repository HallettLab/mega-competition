...
unique(brho$plot) ## no plot NAs
sort(unique(brho$plot)) ## correct plot #'s present
unique(brho$sub) ## no sub NAs
sort(unique(brho$sub)) ## no sub 10 or 12
unique(brho$bkgrd) ## TWIL-U and THIR-U still in this
...
brho_NC <- c(11698, 4794, 9935, 10360, 8781) ## make a vector of the 4 incomplete samples and the 1 missing sample
...
sort(unique(brho$inflor.g)) ## ranges from 0.004 to 3.051; these all seem like reasonable values.
  ...
  ## "1.6847g"
brho_inflor.g_tofix <- brho %>%
  filter(inflor.g == "1.6847g")
## unique ID 5568
...
brho_temp <- brho %>%
  ...
  filter(!unique.id %in% brho_NC) %>% ## remove the incomplete & missing phytos
...
rm(list = c("brho_complete_check_nas", "brho_complete_check_no", "brho_dens_nas", "brho_inflor_na", "brho_inflor.g_tofix", "brho_phyto_zero", "unique.check", "unique.check.nas"))
